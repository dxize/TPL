...
`FunctionsTests`
### Определение и вызов
- [] Функция без аргументов: `func int get_five() { return 5; }`
- [] Функция с одним аргументом: `func int square(int x) { return x * x; }`
- [] Несколько аргументов разных типов: `func string repeat(string s, int n) { ... }`
- [] Процедура (без возвращаемого значения): `proc say_hi() { print("hi"); }`
- [] Вызов функции в выражении: `print(square(5) + 10)`;
- [] Вложенные вызовы: `print(square(get_five()))`;

### Области видимости
- [] Локальные переменные функции не пересекаются с main.
- [] Функция видит глобальные переменные и константы.
- [] Передача по значению: присваивание параметру внутри функции не меняет переменную снаружи.
- [] Shadowing: Параметры функции перекрывают глобальные переменные с тем же именем.

### Неявное приведение в return
- [] Функция bool возвращает 1 (станет true).
- [] Функция bool возвращает 1.5 (приводится к true).
- [] Функция bool возвращает "" (станет false).

### Негативные тесты(функции, семантика)
- [] Вызов до объявления: => SemanticException
- [] Прямая рекурсия: => SemanticException
- [] Взаимная рекурсия: => SemanticException
- [] Внутри функции объявлена переменная с именем параметра. => SemanticException
- [] Ошибка в количестве аргументов: => SemanticException
- [] Ошибка в несовпадении типов передаваемых аргументов:  => SemanticException
- [] Несоответствие возвращаемого типа: => SemanticException
- [] Отсутствие return в func: => SemanticException
- [] Повторное объявление функции: => SemanticException
