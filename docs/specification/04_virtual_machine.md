...
```mermaid
---
title: Структура кадра вызова DEA+E
---
flowchart TB
    Frame["Кадр вызова"]

    RA["Адрес возврата"]
    Info["Сведения о вызываемой функции/процедуре"]

    Scopes["Стек локальных таблиц переменных\n(текущая область + родительские области)"]
...
5. Стек кадров вызова, используемый при вызове пользовательских функций и процедур.
   ...
   - Каждый кадр вызова хранит адрес возврата, сведения о вызываемой функции и стек локальных таблиц переменных, соответствующих вложенным лексическим областям видимости внутри этой функции / процедуры;
...
15. `CallUser [Name]` вызывает пользовательскую функцию или процедуру
    ...
    - Аргументы передаются через стек, где `EVAL[^1]` — последний из `N` аргументов, а `EVAL[^N]` — первый аргумент.
    - Прежнее значение указателя инструкций и стек локальных таблиц переменных сохраняются в стек кадров вызова.
...
28. `EnterScope` открывает новую лексическую область видимости
    - Создаёт новую пустую таблицу переменных в текущем кадре вызова.
    - Новая область становится текущей и получает ссылку на родительскую область.
    - Используется при входе во вложенный блок кода.

29. `LeaveScope` закрывает текущую лексическую область видимости
    - Удаляет текущую таблицу переменных текущего кадра вызова.
    - После удаления текущей становится родительская область видимости.
    - Используется при выходе из вложенного блока кода.
    - Если текущая область видимости является единственной в кадре вызова, она не удаляется.